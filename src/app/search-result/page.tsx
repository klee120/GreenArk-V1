...
const SearchResultContent = () => {
    ...
    // Important: move this setCurrentPage out of render (not inside here)
    // if you need to reset the page when filter changes, use an effect instead
    ...
    const handleSearch = (value: string) => {
        router.push(`/search-result?query=${value}`);
        setSearchKeyword('');
        setCurrentPage(0); // reset to first page when new search
    };
    ...
    return (
        <Suspense fallback={
            <div className="flex justify-center items-center h-screen">
                <div className="text-lg font-semibold text-gray-500">Loading search results...</div>
            </div>
        }>
            <SearchResultContent />
        </Suspense>
